...
from message_processing import Message, extract_tokens, parse_text, read_data
...
def get_tfidf_matrix(corpus: list, min_df=0, max_df=1, ngram_range=(1, 3)):
...
def all_results():
    topics = list(map(lambda x: x['text'], json.load(open('data/processed/all_topics.json', encoding='utf-8'))))
    porter = PorterStemmer()
    my_stopwords = stopwords.words('english')
...
def selection(input_, threshold, output_fn=None, confirmed_fn=None, selector=manual_selector):
    ...
    if isinstance(input_, str):
        data = json.load(open(input_, 'r', encoding="utf8"))
    confirmed = defaultdict(lambda: {"origin": "", "similars": []})
    if confirmed_fn is not None:
        confirmed.update(json.load(open(confirmed_fn, 'r', encoding="utf8")))
...
def f_score(true_answers: defaultdict, dataset: defaultdict, print_stats=False):
    ...
    for key, item in dataset.items():
        ...
        for similar_item in item['similars']:
            their = map(lambda x: x['text'], true_answers[key]['similars'])
            if similar_item['text'] in their:
                true_positives += 1
            else:
                false_positives += 1
    for key, item in true_answers.items():
        assert isinstance(key, str)
        ours = map(lambda x: x['text'], dataset[key]['similars'])
        for similar_item in item['similars']:
            if similar_item['text'] not in ours:
                false_negatives += 1
...
def main():
    ...
    # normalized = list(map(lambda x: normalize(x, my_stopwords, porter), topics))
    # tfidf_matrix = get_tfidf_matrix(normalized)
    # tfidf_results(topics, tfidf_matrix, filename="tfidf-test.json")
    #dataset.update(
    #    json.loads(
    #        json.dumps(
    #            selection('data/vectorization/tfidf.json', 0.79, selector=threshold_selector)
    #        )
    #    )
    #)
    ...
    for threshold in range(0, 50):
        ...
        dataset.update(
            json.loads(
                json.dumps(
                    selection('data/vectorization/tfidf.json', threshold / 100, selector=threshold_selector))))
        pr, rc, fs = f_score(true_answers, dataset)
        # print(threshold, pr, rc, fs)
        precision.append(pr)
        recall.append(rc)
        f_scores.append(fs)
    ...
    plt.ylabel("Значение меры")
    plt.plot(thresholds, precision, label='precission')
    plt.plot(thresholds, recall, label='recall')
    plt.plot(thresholds, f_scores, label='f_score')
    plt.legend()
    print(f"MAX-F_SCORE {max(f_scores)} on {thresholds[f_scores.index(max(f_scores))]}")
    print(f"Prec: {precision[f_scores.index(max(f_scores))]}")
    print(f"Recl: {recall[f_scores.index(max(f_scores))]}")
    plt.show()
